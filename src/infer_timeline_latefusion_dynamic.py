...
"""
Dynamic inference for late-fusion timeline from raw videos.

- Finds TOP/SIDE/SIDE2 videos for a session under raw_video_dir
- Extracts 1 fps grayscale frames (128x128)
- Builds sequences with user-specified --seq_len and --stride
- Loads trained late-fusion model (expects dict input with keys TOP/SIDE/SIDE2)
- Predicts probs -> merges into intervals
- Saves intervals to Excel

Example:
  python src/infer_timeline_latefusion_dynamic.py \
    --session "KA010626 F1 B" \
    --model_path results/latefusion/all3_dyn_4_2/best_model.keras \
    --threshold 0.50 \
    --seq_len 4 --stride 2 \
    --out_xlsx results/infer/010626/all3_dyn_4_2/010626_F1_B_pred.xlsx
"""
...
def probs_to_intervals(probs: np.ndarray, threshold: float, seq_len: int, stride: int) -> List[Tuple[float, float, float]]:
...
def main() -> None:
    ...
    X_seq = {v: make_sequences(X_frames[v], args.seq_len, args.stride) for v in views}
    ...
    intervals = probs_to_intervals(probs, threshold=float(args.threshold), seq_len=args.seq_len, stride=args.stride)
    df = pd.DataFrame(intervals, columns=["start_sec", "end_sec", "mean_prob"])
    if not df.empty:
        df["start_hms"] = df["start_sec"].apply(sec_to_hhmmss)
        df["end_hms"] = df["end_sec"].apply(sec_to_hhmmss)
